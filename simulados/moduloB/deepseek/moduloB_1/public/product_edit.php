...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete'])) {
        // Exclusão permanente só permitida se produto já estiver oculto
        if ($product['hidden'] == 1) {
        ...
        // Validação GTIN (se foi alterado)
        ...
        if (!ctype_digit($new_gtin) || (strlen($new_gtin) != 13 && strlen($new_gtin) != 14)) {
            ...
            // Verificar se o GTIN já existe (exceto o atual)
...
<body>
    ...
    <form method="post" onsubmit="return confirm('Exclusão permanente! Tem certeza?')">
        ...
        <?php if (!$product['hidden']): ?>
            <p style="color:gray">Produto visível não pode ser excluído. Oculte-o primeiro.</p>
        <?php endif; ?>
    </form>
</body>
